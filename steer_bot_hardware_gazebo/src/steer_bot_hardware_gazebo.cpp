...
bool SteerBotHardwareGazebo::initSim(
  ...
  ROS_DEBUG_STREAM_THROTTLE(1, "wheel_separation_w = " << wheel_separation_w_);
  ROS_DEBUG_STREAM_THROTTLE(1, "wheel_separation_h = " << wheel_separation_h_);
  ...
  for (size_t i = 0; i < cmd_handle_names.size(); ++i) {
    ...
    if (!getJointLimits(urdf_joint, limits) ||
      ...
      ROS_DEBUG_STREAM_THROTTLE(1, "Joint limits will be enforced for joint '"
                                      << name << "'.");
    }
  }
  ...
  for (size_t i = 0; i < virtual_jnt_cnt_; ++i) {
    ...
    ROS_DEBUG_STREAM_THROTTLE(1, "Trying to set pid param of '"
                                    << jnt_name << " ' at PID proc in init()");
    if (!pids_[i].init(joint_nh)) {
      ROS_DEBUG_STREAM_THROTTLE(1, "Faied to set pid param of '"
                                      << jnt_name
                                      << " ' at PID proc in init()");
      return false;
    }
    ROS_DEBUG_STREAM_THROTTLE(1, "Succeeded to set pid param of '"
                                    << jnt_name << " ' at PID proc in init()");
  }
...
void SteerBotHardwareGazebo::writeSim(ros::Time time, ros::Duration period) {
  ...
  for (int i = 0; i < sim_joints_.size(); i++) {
    ...
    if (gazebo_jnt_name == virtual_rear_wheel_jnt_names_[INDEX_RIGHT]) {
      ...
      // == 0)
      //{
      //   ROS_DEBUG_STREAM_THROTTLE(1,"rear_wheel_jnt_vel_cmd_ = " <<
      //   rear_wheel_jnt_vel_cmd_);
      //   ROS_DEBUG_STREAM_THROTTLE(1,"virtual_rear_wheel_jnt_vel_[INDEX_RIGHT]
      //   = " << virtual_rear_wheel_jnt_vel_[INDEX_RIGHT]);
      //   ROS_DEBUG_STREAM_THROTTLE(1,"error[INDEX_RIGHT] " <<
      //   rear_wheel_jnt_vel_cmd_ - virtual_rear_wheel_jnt_vel_[INDEX_RIGHT]);
      //   ROS_DEBUG_STREAM_THROTTLE(1,"command[INDEX_RIGHT] = " << eff_cmd);
      // }
      ...
      //   ROS_DEBUG_STREAM_THROTTLE(1,"rear_wheel_jnt_vel_cmd_ = " <<
      //   rear_wheel_jnt_vel_cmd_);
      //   ROS_DEBUG_STREAM_THROTTLE(1,"virtual_rear_wheel_jnt_vel_[INDEX_LEFT] =
      //   " << virtual_rear_wheel_jnt_vel_[INDEX_LEFT]);
      //   ROS_DEBUG_STREAM_THROTTLE(1,"error[INDEX_LEFT] " <<
      //   rear_wheel_jnt_vel_cmd_ - virtual_rear_wheel_jnt_vel_[INDEX_LEFT]);
      //   ROS_DEBUG_STREAM_THROTTLE(1,"command[INDEX_LEFT] = " << eff_cmd);
      // }
      sim_joints_[i]->SetVelocity(0, rear_wheel_jnt_vel_cmd_);
    } else if (gazebo_jnt_name == virtual_front_wheel_jnt_names_[INDEX_RIGHT]) {
      // const double eff_cmd = 2*ComputeEffCommandFromVelError(INDEX_RIGHT,
    ...
    // steers
      ...
      ROS_DEBUG_STREAM_THROTTLE(1, "front_steer_jnt_pos_ '"
      ...
      if (enable_ackermann_link_) {
        ...
        ROS_DEBUG_STREAM_THROTTLE(1, "ackermann steer angle: " << pos_cmd
                                                              << " at RIGHT");
      } else {
      ...
      if (enable_ackermann_link_) {
        const double h = wheel_separation_h_;
        const double w = wheel_separation_w_;
        if (fabs(front_steer_jnt_pos_cmd_) >= M_PI_2)
          front_steer_jnt_pos_cmd_ = M_PI_2;
        try {
          pos_cmd = atan2(2 * h * tan(front_steer_jnt_pos_cmd_),
                          2 * h - w / 2.0 * tan(front_steer_jnt_pos_cmd_));

        } catch (const std::exception &e) {
          ROS_DEBUG_STREAM_THROTTLE(1, "EXECPTION" << e.what());
        }
        ROS_DEBUG_STREAM_THROTTLE(1, "ackermann steer angle: " << pos_cmd
                                                              << " at LEFT");
      } else {
      ...
      // do nothing
      ROS_DEBUG_STREAM_THROTTLE(1, "DO NOTHING");
    }
  }
  ...
  front_steer_jnt_pos_ = front_steer_jnt_pos_cmd_;
  if (log_cnt_ % 500 == 0) {
    ROS_DEBUG_STREAM_THROTTLE(1, "front_steer_jnt_pos_ '" << front_steer_jnt_pos_
                                                         << " ' at writeSim()");
  }
  ...
  rear_wheel_jnt_vel_ = 0.5 * (virtual_rear_wheel_jnt_vel_[INDEX_RIGHT] +
                               virtual_rear_wheel_jnt_vel_[INDEX_LEFT]);
  if (log_cnt_ % 500 == 0) {
    ROS_DEBUG_STREAM_THROTTLE(1, "rear_wheel_jnt_pos_ '" << rear_wheel_jnt_pos_
                                                        << " ' at writeSim()");
    ROS_DEBUG_STREAM_THROTTLE(1, "rear_wheel_jnt_vel_ '" << rear_wheel_jnt_vel_
                                                        << " ' at writeSim()");
  }
}
...
void SteerBotHardwareGazebo::RegisterJointStateInterfaceHandle(
  ...
  ROS_DEBUG_STREAM_THROTTLE(
      1, "Registered joint '" << _jnt_name << " ' in the JointStateInterface");
}
...
void SteerBotHardwareGazebo::RegisterCommandJointInterfaceHandle(
  ...
  ROS_DEBUG_STREAM_THROTTLE(1, "Registered joint '"
                                  << _jnt_name
...
double SteerBotHardwareGazebo::ComputeEffCommandFromVelError(
  ...
  ROS_DEBUG_STREAM_THROTTLE(1, "vel_error = " << vel_error);
  if (fabs(vel_error) < 0.1) {
    vel_error = 0.0;
    ROS_DEBUG_STREAM_THROTTLE(1, "too small. vel_error <- 0");
  } else
    ROS_DEBUG_STREAM_THROTTLE(1, "not small. ");

  const double command = pids_[_index].computeCommand(vel_error, _period);
  ROS_DEBUG_STREAM_THROTTLE(1, "command =" << command);
